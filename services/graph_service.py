...
class Neo4jGraphService:
    ...
    async def create_node(self, node: GraphNode) -> Dict[str, Any]:
        ...
        try:
            with self.driver.session(database=settings.neo4j_database) as session:
                ...
                return {"success": True, "node_id": node.id, "labels": node.labels}
        except Exception as e:
    ...
    async def create_relationship(
        ...
        try:
            with self.driver.session(database=settings.neo4j_database) as session:
                ...
                logger.info(
                    f"Successfully created relationship: {relationship.start_node} -> {relationship.end_node}"
                )
    ...
    async def find_relationships_by_type(
    ...
    async def delete_source_data(self, source_id: str) -> Dict[str, Any]:
        """Delete all nodes and relationships for a specific source"""
        if not self._connected:
            raise Exception("Not connected to Neo4j")

        try:
            with self.driver.session(database=settings.neo4j_database) as session:
                # Delete all nodes that belong to this source
                # This will also delete all relationships due to DETACH DELETE
                query = """
                MATCH (n)
                WHERE n.source_id = $source_id
                WITH n, count(*) as node_count
                DETACH DELETE n
                RETURN node_count
                """
                ...
                logger.info(
                    f"Deleted {nodes_deleted} nodes and {relationships_deleted} relationships for source {source_id}"
                )
            ...
            logger.error(f"Failed to delete source data: {e}")
            return {
                "success": False,
                "error": str(e),
                "nodes_deleted": 0,
    ...
    async def delete_file_nodes(self, source_id: str, file_path: str) -> Dict[str, Any]:
        """Delete all nodes related to a specific file in a source"""
        if not self._connected:
            raise Exception("Not connected to Neo4j")
        ...
        try:
            with self.driver.session(database=settings.neo4j_database) as session:
                ...
                # Match by source_id and file_path property
                query = """
                MATCH (n)
                WHERE n.source_id = $source_id
                  AND (n.file_path = $file_path OR n.path = $file_path)
                DETACH DELETE n
                RETURN count(n) as deleted_count
                """
                result = session.run(
                    query, {"source_id": source_id, "file_path": file_path}
                )
                summary = result.consume()
                ...
                logger.debug(
                    f"Deleted {nodes_deleted} nodes and {relationships_deleted} relationships for file {file_path}"
                )
            ...
            for stat_name, query in stats_queries:
                ...
                if result.raw_result and len(result.raw_result) > 0:
                    if stat_name in ["total_nodes", "total_relationships"]:
                        ...
                        stats[stat_name] = result.raw_result[0].get(
                            stat_name.split("_")[1], []
                        )
    ...
    async def _batch_create_nodes_standard(
