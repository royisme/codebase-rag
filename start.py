...
def check_dependencies():
    ...
    if (
        ...
        or settings.embedding_provider == "openrouter"
    ):
...
def main():
    ...
    if not settings.debug or input(
        "skip service dependency check? (y/N): "
    ).lower().startswith("y"):
        logger.info("skip service dependency check")
    else:
        if not wait_for_services():
            logger.error(
                "service dependency check failed, continue startup may encounter problems"
            )
